...
def load_config(path: str | Path) -> AppConfig:
    ...
    strategy_config = StrategyConfig(
        ...
        dynamic_top_k=bool(strategy.get("dynamic_top_k", False)),
        dynamic_top_k_min=int(strategy.get("dynamic_top_k_min", 3)),
        dynamic_top_k_max=int(strategy.get("dynamic_top_k_max", 7)),
        dynamic_top_k_disp_low=float(strategy.get("dynamic_top_k_disp_low", 0.04)),
        dynamic_top_k_disp_high=float(strategy.get("dynamic_top_k_disp_high", 0.10)),
        ...
        cluster_constraint=bool(strategy.get("cluster_constraint", False)),
        max_per_cluster=int(strategy.get("max_per_cluster", 2)),
        market_state_mode=str(strategy.get("market_state_mode", "binary")),
        three_state_strong_exposure=float(strategy.get("three_state_strong_exposure", 1.00)),
        three_state_neutral_exposure=float(strategy.get("three_state_neutral_exposure", 0.60)),
        three_state_weak_exposure=float(strategy.get("three_state_weak_exposure", 0.25)),
        three_state_trend_weight=float(strategy.get("three_state_trend_weight", 0.35)),
        three_state_dispersion_weight=float(strategy.get("three_state_dispersion_weight", 0.25)),
        three_state_momentum_weight=float(strategy.get("three_state_momentum_weight", 0.25)),
        three_state_breadth_weight=float(strategy.get("three_state_breadth_weight", 0.15)),
        three_state_strong_threshold=float(strategy.get("three_state_strong_threshold", 0.25)),
        three_state_weak_threshold=float(strategy.get("three_state_weak_threshold", -0.10)),
        turnover_budget=float(strategy["turnover_budget"]) if "turnover_budget" in strategy else None,
        turnover_budget_window=int(strategy.get("turnover_budget_window", 252)),
        staggered_rebalance=bool(strategy.get("staggered_rebalance", False)),
        staggered_n_tranches=int(strategy.get("staggered_n_tranches", 4)),
    )
    return AppConfig(
